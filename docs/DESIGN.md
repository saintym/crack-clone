...
**프롬프트 (`ImagesContributor`, IMAGES slot, order 0, name `images`)** — 태그와 설명만 넣고 URL은 넣지 않는다(토큰 절약). 최대 개수는 `crack.image.prompt-max-entries`(기본 50, 위에서부터). T27에서 두 부분으로 나눴고, T30에서 인물 부분을 본문 인라인 안내로 바꿨다.
1. **활성 인물 변형 목록**: 활성 인물(§6.2, `ActiveCharacterSelector`) 중 카탈로그에 항목이 있는 인물만, `설월 — 기본, 당황, 분노`처럼 쓸 수 있는 변형 이름을 준다.
   **AI는 이 이름으로 태그를 조립해 본문에 직접 넣는다**(`{{img:설월_당황}}`, D34). 놓을 자리는 **그 인물의 대사·등장 묘사 바로 앞 줄**이고, 말하는 인물이 바뀔 때마다 새로 넣되 **한 응답에 3개까지**다. 활성 인물은 기억 기록 회차 동안 고정되므로(D30) 이 목록도 회차 안에서 바뀌지 않는다(캐시 접두사 유지).
2. **장면·배경 태그 목록**: 인물 태그가 아닌 항목만. "필요할 때만 `{{img:태그}}`를 한 줄에 단독으로, 한 응답에 1개까지" 안내한다. **인물 이미지는 이 목록에 넣지 않는다**(1번이 맡는다).

**프론트 표시 규칙**
- **본문 태그(T20, T30에서 확장):** AI 말풍선에서만 바꾼다. `{{img:태그}}`는 **나온 자리에서** 이미지가 된다.
  - 한 줄에 단독으로 있으면 그 줄이 이미지가 된다. **줄 안에 섞여 있어도 그 자리에서 줄을 쪼갠다**(D34). AI가 대사 앞에 붙여 쓴 경우를 받아 준다.
  - 태그 이름으로 쓸 수 없는 글자가 들어 있으면(`{{img: }}` 등) 그 태그만 지운다.
  - 스트리밍 중 아직 덜 들어온 마지막 줄의 태그는 보이지 않는다. 카탈로그를 받기 전이나 이미지를 불러오지 못하면 숨긴다.
  - **사이에 글 없이 같은 이미지가 이어지면 한 번만 보여 준다.** 글을 사이에 두고 같은 인물이 다시 나오면 다시 보여 준다.
- **인물 이미지 위치(T30, D34):** 인물 이미지는 **그 인물이 말하는 자리 바로 앞**에 온다. 한 말풍선 안에서 화자가 바뀌면 이미지도 그만큼 들어간다. 말풍선 맨 위에 한 장만 띄우는 방식은 폴백으로만 남았다.
- **인물 이미지 폴백(T27, D31):** 본문에 카탈로그에 있는 `{{img:…}}`가 **하나도 없을 때만**, 렌더 시 `MessageView.speaker`로 카탈로그에서 골라 말풍선 맨 위에 보여 준다. AI가 본문 태그를 빠뜨린 턴과 T30 이전에 저장된 메시지를 위한 것이다.
  ...
  - 첫 줄 `[인물: 이름/변형]` 태그(§5.3)와 `speaker`·`speaker_variant` 컬럼은 이 폴백을 위해 그대로 남는다.
