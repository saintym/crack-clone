...
> **이어받는 세션은 [docs/HANDOFF.md](docs/HANDOFF.md)부터 읽는다.** 현재 상태, 사용자가 해야 할 일, 새 작업 추가 절차, 반복된 함정이 정리돼 있다.
...
8. **이 저장소는 공개(public)다.** 비밀값(`application.yml`, API 키, 비밀번호), `data/` 아래 사용자 데이터, 개인 경로를 커밋하지 않는다.
   - **작업 명세·문서·로그에도 비밀값을 적지 않는다.** DB 비밀번호, 인증 비밀번호, 토큰을 적어야 할 상황이면 값 대신 위치만 쓴다(예: "비밀번호는 로컬 `application.yml` 참고"). 실제로 2026-09-26에 작업 명세 두 곳에 DB 비밀번호가 적혀 공개 저장소에 올라간 일이 있었다
   - 에이전트에게 값을 전달할 때는 **대화(프롬프트)로만** 주고 파일에 남기지 않는다
